...
class SparseBrain(sb.core.Brain):
    def compute_forward(self, batch: Dict[str, torch.Tensor], stage: sb.Stage) -> Tuple:
        ...
        speaker_codes = batch.spk_id_encoded[0]

        # 1. Core Encoder + Disentangler
        ...
        if isinstance(self.modules.asr_encoder, sb.nnet.RNN.LSTM):
            enc_out_asr, _ = self.modules.asr_encoder(content_enc_input, lengths=wav_lens)
        else:
            target_tokens, _ = batch.tokens
            enc_out_asr, _, _, _ = self.modules.asr_encoder(content_enc_input, target_tokens, wav_lens)
        logits = self.modules.ctc_lin(enc_out_asr)
        p_ctc = self.hparams.log_softmax(logits)

        # Standard Speaker pass
        spk_emb = self.modules.spk_encoder(z_proj_speaker)
        spk_logits = self.modules.spk_classifier(spk_emb).squeeze(1)

        # --- ADVERSARIAL MUTUAL DISENTANGLEMENT BRANCHES ---
        adv_spk_logits = None
        p_adv_ctc = None

        if stage == sb.Stage.TRAIN:
            # Branch A: Look for speaker leak inside your Content vector
            # (Mean pool over time dimension first if passing to a standard classifier)
            reversed_content = self.modules.grl_content(z_proj_content).mean(dim=1)
            adv_spk_logits = self.modules.adv_spk_classifier(reversed_content).squeeze(1)

            # Branch B: Look for content leak inside your Speaker frames 
            # (Run CTC on sequence frames before ECAPA pools them)
            reversed_speaker_seq = self.modules.grl_speaker(z_proj_speaker)
            adv_ctc_logits = self.modules.adv_ctc_lin(reversed_speaker_seq)
            p_adv_ctc = self.hparams.log_softmax(adv_ctc_logits)

        # Standard Decoding setup
        pred_hyps = None
        if stage == sb.Stage.VALID:
            pred_hyps = sb.decoders.ctc_greedy_decode(p_ctc, wav_lens, blank_id=self.hparams.blank_index)
    ...
    def compute_objectives(self, predictions: Tuple, batch: Dict[str, torch.Tensor], stage: sb.Stage) -> torch.Tensor:
        ...
        if stage == sb.Stage.TRAIN:
            batch_aam_loss = self.hparams.spk_aam_loss(spk_logits, spk_targets) * self.hparams.spk_aam_loss_weight
            
            # --- COMPUTE ADVERSARIAL LOSSES ---
            # 1. Adversarial Speaker Loss (AAM / CE Cross Entropy metric on Content)
            loss_adv_spk = self.hparams.adv_spk_loss(adv_spk_logits, spk_targets.squeeze()) * self.hparams.adv_spk_weight
            
            # 2. Adversarial Content Loss (CTC metric on Speaker)
            loss_adv_ctc = self.hparams.ctc_cost(p_adv_ctc, tokens, wav_lens, tokens_lens, reduction=self.hparams.loss_reduction) * self.hparams.adv_ctc_weight
        ...
        if stage == sb.Stage.TRAIN:
            ...
            log_stats = {
                ...
                "loss_adv_spk": loss_adv_spk.item(),
                "loss_adv_ctc": loss_adv_ctc.item(),
...
def dataio_prepare(hparams: dict, tokenizer: SentencePiece) -> Tuple:
    ...
    def label_pipeline(spk_id):
        ...
        spk_id_encoded = spk_label_encoder.encode_sequence_torch(
            [spk_id_prefix], allow_unk=True
        )
